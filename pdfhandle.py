“””
Clean PDF Parser for Contract Analysis

A simplified PDF processing module that extracts text from PDFs, determines layout structure,
and organizes content into structured paragraphs with optional embeddings.

Key Features:

- PDF text extraction and quality validation
- Layout detection (single/double column)
- Intelligent paragraph extraction with cross-page continuity
    ...
    for page in pages_data:
        ...
        if previous_paragraph:
            ...
            if should_continue and paragraphs:
                # Merge with first paragraph of current page
                paragraphs[0] = prev_text + " " + paragraphs[0]
            elif should_continue:
                # Add orphaned paragraph if current page has no content
                paragraphs.insert(0, prev_text)
            previous_paragraph = None
        
        # Check if last paragraph should continue to next page
        if paragraphs:
            last_para = paragraphs[-1]
            should_continue = self._should_continue_paragraph(last_para)
            if should_continue:
                previous_paragraph = (last_para, True)
                paragraphs = paragraphs[:-1]  # Remove from current page
...
def _sort_blocks_by_position(self, text_blocks: List[Dict], page_height: float, layout_type: str) -> List[Dict]:
    ...
    if layout_type == "double_column":
        # For double column, sort by column first, then by vertical position
...
def _group_blocks_into_paragraphs(self, sorted_blocks: List[Dict]) -> List[str]:
    ...
    for i in range(1, len(sorted_blocks)):
        ...
        # Check if blocks should be part of same paragraph
    ...
    # Join short paragraphs with following ones
...
def _join_short_paragraphs(self, paragraphs: List[str]) -> List[str]:
    ...
    for i in range(1, len(paragraphs)):
        ...
        # Check if current paragraph should be joined with next
...
def _generate_embeddings(self, structured_pages: List[Dict]) -> Dict[str, Any]:
    ...
    try:
        # Collect all paragraphs with their page/paragraph indices
        all_paragraphs = []
        paragraph_map = []
        ...
        return {
            "model": self.embedding_model._model_name,
...
def parse_single_pdf(pdf_path: str, output_path: str = None, generate_embeddings: bool = False) -> Dict[str, Any]:
“””
...
def parse_pdf_directory(input_dir: str, output_dir: str, generate_embeddings: bool = False) -> List[Dict[str, Any]]:
“””
...
if os.path.exists(pdf_file):
    result = parse_single_pdf(pdf_file, json_output, generate_embeddings=True)
...
# Example 2: Parse all PDFs in a directory
# input_directory = "contracts/"
# output_directory = "extracted/"
# results = parse_pdf_directory(input_directory, output_directory, generate_embeddings=True)
