...
results in a structured JSON format or plain text file.
...
- Text file output with paragraphs separated by double newlines
"""
...
class PDFHandler:
    ...
    def save_to_txt(self, result: Dict[str, Any], output_path: str) -> str:
        """
        Save the parsed paragraphs to a text file with double newlines between paragraphs.
        
        Args:
            result: Dictionary with the processed content (from process_pdf)
        ...
        text_content = "\n\n".join(self.clean_text(para) for para in all_paragraphs if para.strip())
...
def process_directory(input_dir: str, output_dir: str, generate_embeddings: bool = False, save_txt: bool = False) -> List[Dict[str, Any]]:
    """
    Process all PDF files in a directory, with option to save as text files.
    
    Args:
        input_dir: Directory containing PDF files
        output_dir: Directory to save JSON and optional text results
        generate_embeddings: Whether to generate embeddings
        save_txt: Whether to save parsed content as text files
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.pdf'):
            ...
            if save_txt:
                handler.save_to_txt(result, output_txt_path)
                print(f"Saved text output to {output_txt_path}")
...
if __name__ == "__main__":
    ...
    output_json_path = "extracted/sample_contract.json"  # Path for the output JSON
    output_txt_path = "extracted/sample_contract.txt"    # Path for the output text
    ...
    if os.path.isfile(input_path):
        ...
        handler.save_to_json(result, output_json_path)
        handler.save_to_txt(result, output_txt_path)
        print(f"Processed {input_path}, saved JSON to {output_json_path}, and text to {output_txt_path}")
