# database/helpers.py - Safe database helper functions for PDF handler and Streamlit
...
from database.models import (
    ...
    generate_file_hash, generate_session_id
)
...
# USER OPERATIONS
def get_or_create_user_by_session(session_id: str, username: str = None) -> Dict[str, Any]:
        ...
        with db_manager.get_session() as session:
            ...
            if user:
                ...
                return {
                    ...
                    'created_at': DatabaseHelper.safe_convert_datetime(user.created_at),
                    'last_active': DatabaseHelper.safe_convert_datetime(user.last_active),
                    'is_new': False
                }
            ...
            return {
                ...
                'created_at': DatabaseHelper.safe_convert_datetime(new_user.created_at),
                'last_active': DatabaseHelper.safe_convert_datetime(new_user.last_active),
                'is_new': True
            }
            
    except Exception as e:
        logger.error(f"Error in get_or_create_user_by_session: {e}")
...
def update_user_activity(session_id: str) -> bool:
    ...
    try:
        with db_manager.get_session() as session:
            ...
            if user:
                ...
                session.flush()
                logger.info(f"Updated activity for user: {user.username}")
                return True
            return False
    except Exception as e:
...
# PDF OPERATIONS
def store_pdf_from_upload(file_data: bytes, filename: str, user_session_id: str, 
